...
export class HttpUtil {
        ...
        try {
            LoggingUtil.log.debug(`Rest API  end point ${updatedConfig.baseURL} , ${updatedConfig.url}`);
            let response = await Axios.request(updatedConfig);
            LoggingUtil.log.debug(`Result ${JSON.stringify(response.data)}`);
            return response.data;

        } catch (e) {
            LoggingUtil.log.error(`Exception Rest API  end point ${JSON.stringify(config)}`, e);
            return e;
        }
    }
